...
게다가! 가상 함수는 런타임에 결정되므로 인라인화가 안 된다.
하지만, final 키워드를 붙여주면 컴파일러는 '이 클래스 밑으로는 절대 이 함수를 오버라이딩 할 수 없구나'라고 알게 된다.
그럼 이 클래스 포인터로 호출할 때는 무조건 이 코드가 실행되겠네?라고 확신할 수 있다.

**이릍 통해 컴파일러는 가상 함수 테이블을 거치는 오버라이딩 메커니즘을 생략하고,
가상 함수임에도 불구하고 인라인 화가 가능하다!**
