...
직므의 경우 두 checkOut 함수는 C++ 규칙에 의한 일치도가 서로 같기 때문에, 최적 일치 함수가 결정되지 않는다.
...
단일 상속과 비교해서 사용하기에도 좀더 복잡하고 이해하기에도 좁더 복잡하다는 것은 사실이므로,
...
> 가상 상속을 스면 크기 비용, 속도 비용이 늘어나며, 초기화 및 대입 연산의 복잡도가 커진다.
...
이는 "Empty Class 타입의 멤버를 가질 때 독립적인 주소를 가질 필요없다."는 의미를 가진다.
